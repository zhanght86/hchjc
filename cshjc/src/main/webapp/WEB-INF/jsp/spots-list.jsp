...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 采集点管理 <span
        class="c-gray en">&gt;</span> 采集点列表 <a class="btn btn-success radius r"
...
<form id="formfordata" class="page-container" enctype="multipart/form-data">
	...
	<div class="text-c">
        ...
        <option value="采集点1" selected="">采集点1</option>
        <option value="采集点2">采集点2</option>
    </select>
    ...
    <div class="cl bg-1 bk-gray mt-20">
        <span class="l">
            ...
            <a class="btn btn-primary radius" data-title="添加"
               onclick="slide_add('添加采集点','<%=basePathAdmin%>/spots.do?method=create',0,0)" href="javascript:;">
                <i class="Hui-iconfont">&#xe600;</i> 添加采集点
            </a>
        </span>
    </div>
    <div class="mt-20">
...
//        "bStateSave": true, //状态保存
        ...
        "processing": true,
        "serverSide": true,
        "sAjaxSource": "<%=basePathAdmin%>/spots.do?method=querylist",//这个是请求的地址
        ...
        "columns": [
            {
                ...
                return '<input type="checkbox" name="checkBoxforDel" class="checkBoxforDel" value="' + row.spotId + '" />';
            }
            },
            {
                "data": "spotId",
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    ...
                    var uril = "'<%=basePathAdmin%>/spots.do?method=update'";

                    return str+'<a title="编辑" href="javascript:;" onclick="team_edit(\'编辑：' + row.spotName + '\','+uril+',' + row.spotId + ')" style=text-decoration:none><i class=Hui-iconfont>&#xe6df;</i></a><a title="删除" href="javascript:;" onclick="team_del(this,' + row.spotId + ')" class=ml-5 style=text-decoration:none><i class=Hui-iconfont>&#xe6e2;</i></a>';
                }
            }
        ]
    });
    ...
    function team_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            $("#formfordata").ajaxSubmit({
                url: "<%=basePathAdmin%>/spots.do?method=delete",//这个就是请求地址对应sAjaxSource
                data: {
                    "spotId": id
                },//这个是把datatable的一些基本数据传给后台,比如起始位置,每页显示的行数
                type: 'get',
    ...
    function datadel() {
        ...
        layer.confirm('确认要删除这' + list.length + '条吗？', function () {
            $("#formfordata").ajaxSubmit({
                url: "<%=basePathAdmin%>/spots.do?method=deletelist",//这个就是请求地址对应sAjaxSource
                type: 'get',
                dataType: "json",
                async: true,
                success: function (data) {
    ...
    function slide_updown(obj, id, status) {
        ...
        layer.confirm(strq, function (index) {
            $("#formfordata").ajaxSubmit({
                url: "<%=basePathAdmin%>/spots.do?method=change",//这个就是请求地址对应sAjaxSource
                data: {
                    "id": id,
